...
if ($_POST) {
    ...
    if ($password == $cpassword) {
        $error = '3';
        $result = $database->query("select doctor.docid from doctor inner join users on doctor.docemail=users.email where users.email='$email';");
        ...
        if ($id2 != $id) {
            ...
            $sql1 = "update users set email='$email' where email='$oldemail' ;";
            $database->query($sql1);
